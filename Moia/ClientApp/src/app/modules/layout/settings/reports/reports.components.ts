...
import { BranshDto, CommitteeIdName, DepartmentDto, FilterData, MainRoleNameId, MainUserDto, NameIdViewModel, OrderTransfereDTO, StatisticalsOrders, SwaggerClient, UserDto, UserType, ViewerPaginationOfMainUserDto, ViewerPaginationOfStatisticalsOrders, ViewerPaginationOfWitness, Witness } from 'src/app/Shared/Services/Swagger/SwaggerClient.service';
...
export class ReportsComponent extends DefaultListComponent<ViewerPaginationOfStatisticalsOrders, StatisticalsOrders> {
  ...
  relatedUsers:NameIdViewModel[]=[]
  selectedUserToTransfereOrders:NameIdViewModel
  ...
  OrderTransfere:OrderTransfereDTO = {toUser : 0 , fromUser:0 } as OrderTransfereDTO
  ...
  GetRelatedUsers(user:any) {
    this.swagger.apiLookupGetRelatedUsersGet(user.id).subscribe(
      res => {
        this.relatedUsers = res
      }
    )
  }
  ...
  SetOrderTransfere() {
    ...
    this.OrderTransfere.toUser = this.selectedUserToTransfereOrders.id ?  this.selectedUserToTransfereOrders.id:0 

    if(this.OrderTransfere.fromUser !=0 && this.OrderTransfere.toUser != 0 )
    this.swagger.apiCommitteeTransfereOrdersPost(this.OrderTransfere).subscribe(
      res => {
        this.OrderTransfere = new OrderTransfereDTO()
        this.display=false

        this.toast.add({
          severity: 'success',
          detail: 'تم ترحيل الطلبات بنجاح',
        });
  ...
  TransfereOrder(user:any){
    this.GetRelatedUsers(user)
      this.display=true
      this.OrderTransfere.fromUser = user.id
  }
  ...
  returnDataFn(): Observable<ViewerPaginationOfStatisticalsOrders> {
    ...
    return this.swagger.apiReportGetStatisticalsOrdersGet(this.Filter.brn,this.Filter.com,this.fromDate,this.toDate,this.Filter.page,this.Filter.pageSize );
  }
