...
@Component({
  ...
  templateUrl: './all-my-orders.component.html',
  styleUrls: ['./all-my-orders.component.scss']
})
export class AllMyOrdersComponent extends DefaultListComponent<ViewerPaginationOfOrderListDto, OrderListDto> {
  ...
  returnDataFn(): Observable<ViewerPaginationOfOrderListDto> {
    ...
    return this.swagger.apiCommitteeGetOrdersForPreviewPost(
    ...
    if (userPermissions.includes('CommitteeManager') ||
      ...
      if (userPermissions.includes('CommitteeManager')) {
        this.OrderCodes.forEach(element => {
          this.swagger.apiMuslimeChangeOrderStatePost(element,
            OrderStatus.Accept,
        ...
        this.OrderCodes.forEach(element => {
          this.swagger.apiMuslimeChangeOrderStatePost(element,
            OrderStatus.Accept,
            OrderStage.Branch,
            this.translate.instant('SendOrderToBranchDescription') + " بواسطة " + this.authService.User$.value['Name']).subscribe(res => {
              this.getData()
            })
